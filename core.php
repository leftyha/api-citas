...
{
    $st = db()->prepare(
        "SELECT TOP 1 license_id, license_uuid, booking_enabled FROM booking_licenses WHERE license_uuid=:u"
    );
    $st->execute(["u" => $uuid]);
    $r = $st->fetch();
    if (!$r) {
...
{
    $st = db()->prepare(
        "SELECT TOP 1 a.*,l.license_uuid FROM booking_appointments a JOIN booking_licenses l ON l.license_id=a.license_id WHERE a.appointment_id=:id"
    );
    $st->execute(["id" => $id]);
    $r = $st->fetch();
    if (!$r) {
    ...
    return [
        ...
        "customer" => [
            "documentMasked" =>
                str_repeat("*", max(0, strlen($doc) - 3)) . substr($doc, -3),
            "phoneMasked" =>
                str_repeat("*", max(0, strlen($phone) - 3)) .
                substr($phone, -3),
        ],
    ];
}
function run(string $route): void
{
    try {
        if ($route === "availability") {
            ...
            license_or_fail($u);
            $st = db()->prepare(
                "SELECT start_at,end_at,status FROM booking_appointments a JOIN booking_licenses l ON l.license_id=a.license_id WHERE l.license_uuid=:u AND CAST(a.start_at AS DATE)=:d"
            );
            ...
            for ($c = $start; $c + $dur * 60 <= $end; $c += $dur * 60) {
                ...
                foreach ($occ as $o) {
                    if (
                        !in_array($o["status"], ["pending", "confirmed"], true)
                    ) {
                        continue;
                    }
        ...
        if ($route === "appointments_create") {
            ...
            $c = $pdo->prepare(
                "SELECT TOP 1 appointment_id,customer_document FROM booking_appointments WHERE license_id=:l AND start_at=:s AND status IN ('pending','confirmed')"
            );
            $c->execute([
                "l" => $lic["id"],
            ...
            if ($ex) {
                if (
                    strtolower((string) $ex["customer_document"]) ===
                    strtolower((string) $in["customerDocument"])
                ) {
                ...
                $q = $pdo->prepare(
                    "INSERT INTO booking_appointments (license_id,customer_document,customer_name,customer_phone,customer_email,start_at,end_at,duration_minutes,service_type,professional_id,notes,status) OUTPUT INSERTED.appointment_id VALUES (:l,:d,:n,:p,:e,:s,:en,:du,:st,:pr,:no,'pending')"
                );
                $q->execute([
                    "l" => $lic["id"],
        ...
        if (strpos($route, "admin_") === 0) {
            ...
            if ($route === "admin_appointments_list") {
                ...
                foreach (
                    [
                        ...
                        "customerDocument" =>
                            "a.customer_document=:customerDocument",
                    ]
                    as $k => $sql
                ) {
                    if (!empty($_GET[$k])) {
                ...
                $sql =
                    "SELECT a.*,l.license_uuid FROM booking_appointments a JOIN booking_licenses l ON l.license_id=a.license_id" .
            ...
            if (
                $route === "admin_appointments_confirm" ||
                $route === "admin_appointments_cancel"
            ) {
                $in = json_in();
                $id = (int) ($in["appointmentId"] ?? 0);
                if ($id <= 0) {
                ...
                $to =
                    $route === "admin_appointments_confirm"
                        ...
                        : "cancelled";
                $pdo->prepare(
                    "UPDATE booking_appointments SET status=:s WHERE appointment_id=:id"
                )->execute(["s" => $to, "id" => $id]);
                ok(
